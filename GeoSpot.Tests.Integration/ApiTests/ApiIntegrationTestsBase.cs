...
public abstract class ApiIntegrationTestsBase : IAsyncLifetime
{
    ...
    protected async Task<UserEntity> AuthorizeClientAsync(HttpClient client)
    {
        ...
        await client.PostAsJsonAsync(ApiUriPaths.AuthUri.SendVerificationCode,
            new SendVerificationCodeRequestDto(phoneNumber));
        
        HttpResponseMessage responseMessage = await client.PostAsJsonAsync(ApiUriPaths.AuthUri.VerifyVerificationCode, 
